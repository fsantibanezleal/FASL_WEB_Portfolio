...
The 3D is a **kinematic animation** of the closed-form engine, **not** a DEM / N-body particle simulation, and there is **no** population-balance breakage model. All operating points are **synthetic-but-realistic** (no plant, PEPT or DEM data); the power magnitude is **calibrated** to a ~1.3 MW textbook reference, not validated against a real mill, and the surrogate's accuracy is measured against the engine, not reality. It is a fast design-intuition studio, not a plant model.
